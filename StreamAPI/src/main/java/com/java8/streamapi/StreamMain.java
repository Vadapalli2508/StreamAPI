...
public class StreamMain {
	...
	public static void main(String[] args) {
		...
		employeeList.stream()
				...
				.forEach(e -> {int sal= (int) (e.getSalary()+(e.getSalary()*0.1));
								e.setSalary(sal);
		...
		empCount.forEach((key1, value1) -> System.out.println(key1 + "-----" + value1));
		...
		empStatus.forEach((key, value) -> System.out.println(key + "--->" + (long) value.size()));
		...
		employeeList.stream().collect(Collectors.groupingBy(Employee::getDepId)).forEach((key, value) -> System.out.println(value.stream().max(Comparator.comparing(Employee::getSalary))));
